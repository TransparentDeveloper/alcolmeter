...
function serializeBlock(node: Node): string[] {
	...
	switch (el.tagName) {
		...
		case 'UL':
			return [serializeList(el, 0, 'ul')];
		case 'OL':
			return [serializeList(el, 0, 'ol')];
		case 'BLOCKQUOTE': {
			const text = inlineOf(el);
...
function serializeInline(node: Node): string {
	...
	switch (el.tagName) {
		...
		case 'B':
		case 'STRONG':
			return wrapMark(children, MarkdownWriter.bold);
		case 'I':
		case 'EM':
			return wrapMark(children, MarkdownWriter.italic);
		case 'S':
		case 'STRIKE':
		case 'DEL':
			return wrapMark(children, MarkdownWriter.strikethrough);
		case 'CODE':
			return wrapMark(children, MarkdownWriter.code);
		case 'A': {
...
function wrapMark(children: string, wrap: (text: string) => string): string {
...
function serializeList(list: HTMLElement, depth: number, type: 'ul' | 'ol'): string {
	const lines: string[] = [];
	...
	for (const child of Array.from(list.children)) {
		...
		for (const sub of Array.from(li.children)) {
			if (sub.tagName === 'UL') lines.push(serializeList(sub as HTMLElement, depth + 1, 'ul'));
			else if (sub.tagName === 'OL') lines.push(serializeList(sub as HTMLElement, depth + 1, 'ol'));
		}
	}
	return lines.join('\n');
}
