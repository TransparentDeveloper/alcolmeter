<script lang="ts">
	...
	const dict = useDictionary(data.terms);

	const jsonLd = JSON.stringify({
		'@context': 'https://schema.org',
		'@type': 'DefinedTermSet',
		name: '알콜미터 전통주 용어사전',
		description: '막걸리·전통주 양조 용어를 입문자 눈높이로 풀어 설명하는 용어사전.',
		url: 'https://alcolmeter.kr/dictionary',
		hasDefinedTerm: data.terms.map((t) => ({
			'@type': 'DefinedTerm',
			name: t.title,
			description: t.summary,
			url: `https://alcolmeter.kr/dictionary/${encodeURIComponent(t.slug)}`
		}))
	});
</script>
