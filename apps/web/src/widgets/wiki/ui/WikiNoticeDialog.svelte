<script lang="ts">
	import { WikiNoticeDialogState, type NoticeContent } from './WikiNoticeDialogState.svelte';

	// 제목+본문+확인 버튼 안내 다이얼로그 (중복 안내·에러 표시 겸용).
	const notice = new WikiNoticeDialogState();

	export function open(content: NoticeContent): void {
		notice.open(content, body);
	}
</script>

{#snippet body({ close }: { close: () => void })}
	<div class="box">
		<h3>{notice.data?.title}</h3>
		<p>{notice.data?.description}</p>
		<div class="actions">
			<button type="button" class="confirm" onclick={close}>확인</button>
		</div>
	</div>
{/snippet}
