...
> *둘러보신 자취를 접고 첫 화면으로 돌아오는 길을 새로 놓았습니다. 그 자리에서 한 걸음 더 물러서시면 이제 문 밖입니다.*

- **돌아오는 길**: 로고를 눌러 첫 화면으로 오시면 발자국을 새로 남기지 않고 처음 들어오신 자리로 돌아갑니다. 그래서 첫 화면에서 뒤로 가기를 한 번만 누르면 곧장 사이트를 나가십니다. 여태는 지나온 화면을 하나씩 되짚느라 여러 번 눌러야 했습니다.
- **맨 위에서 시작**: 그렇게 돌아온 첫 화면은 늘 맨 위에서 펼쳐집니다. 직접 누르신 뒤로 가기는 예전처럼 보시던 자리를 되살립니다.
- **곧장 들어오신 길**: 첫 화면을 거치지 않고 바로 어느 문서로 들어오셨다면 돌아갈 자리가 없으므로 여느 때처럼 이동합니다.

## [1.15.0] — 2026-08-20

> *페이지와 페이지 사이에 없던 한 박자가 새로 들어앉았습니다. 여태 눈 깜빡할 사이에 끝나 버리던 자리를 이제 눈으로 좇으실 수 있습니다.*

- **페이지 전환**: 화면을 옮기면 표지 한 장이 밀려들어와 덮고, 다음 화면이 준비되면 다시 걷힙니다. 앱 안의 모든 이동에 저절로 걸리니 따로 켜실 것은 없습니다.
- **흐르는 방향**: 앞으로 나아가는 이동은 오른쪽에서 왼쪽으로 흐릅니다. 뒤로 가기와 첫 화면으로 돌아오는 이동은 그 반대로 흘러, 어느 쪽으로 가는 길인지 몸이 먼저 압니다.
- **전환 중의 얼굴**: 덮인 동안에는 붉은 판 위에 흰 이름표만 남습니다 (design-system 0.6.0).
- **기다림을 가림**: 다음 화면이 늦게 준비되면 표지가 그만큼 더 머무릅니다. 덜 그려진 화면이나 빈 자리를 보실 일이 없습니다.
- **동작 줄이기 존중**: 기기에서 동작 줄이기를 켜 두셨다면 표지 없이 곧바로 넘어갑니다.

## [1.14.0] — 2026-08-04

> *읽는 자리의 활자를 한 뼘 키워 다시 앉혔습니다. 제목들도 저마다 키를 다시 재었으니, 이제 눈을 가늘게 뜨지 않으셔도 됩니다.*

- **커진 본문**: 알콜위키와 커뮤니티의 본문 글씨를 키웠습니다. 긴 문서도 눈에 힘을 주지 않고 읽힙니다.
- **또렷해진 넷째 제목**: 가장 작은 단계의 제목이 굵게 쓴 본문과 구별되지 않던 것을 바로잡았습니다. 이제 제목 네 단계가 저마다 다른 크기를 가집니다.
- **좁은 화면 손질**: 폭이 좁은 기기에서는 제목이 한 단계씩 내려앉아 화면을 덜 차지합니다. 본문 크기는 그대로 둡니다.
- **표와 인용 상자**: 본문 속 표와 인용 상자도 본문과 같은 크기로 맞췄습니다.
- **약관·정책 문서**: 이용약관·운영정책·개인정보처리방침의 조문도 같은 크기로 키웠습니다.
- **쓰는 화면도 같이**: 글을 쓰는 동안 보이는 글자 크기가 발행된 글과 같아졌습니다.
- **크기 기준 정비**: 제목과 본문 크기를 역할별로 정해, 화면마다 제각각이던 기준을 하나로 모았습니다 (design-system 0.5.0).

## [1.13.0] — 2026-08-04

> *여태 서로 말없이 지켜 오던 약속을 이번엔 글로 받아 적어 두었습니다. 굳이 펼쳐 보실 일이 없기를 바라며 적었습니다만, 어딘가에 적혀 있다는 것만으로 마음이 놓이는 종류의 글입니다.*

- **이용약관**: 계정과 탈퇴, 이용자가 쓰신 글의 권리가 누구에게 있는지, 금지 행위와 게시물 조치, 면책의 범위를 담았습니다. 알콜위키 문서가 CC BY-SA 4.0으로 공개된다는 점과, 탈퇴하셔도 이미 올리신 글은 익명 처리되어 남는다는 점을 여기에 못 박았습니다.
- **운영정책**: 커뮤니티 글 작성 기준, 알콜위키 편집 규칙과 되돌리기·문서 삭제 기준, 신고 방법과 조치 단계, 이의 제기 절차를 따로 정리했습니다. 약관보다 자주 손볼 문서라 자리를 나눴습니다.
- **위키 이용 안내 이전**: 알콜위키의 「이용 안내」는 문을 닫고 운영정책으로 자리를 옮겼습니다. 옛 주소로 찾아오시면 새 자리로 모셔다 드립니다.
- **신고 창구**: 권리를 침해당하셨거나 문제가 되는 글을 보셨다면 운영정책에 적힌 이메일로 알려 주세요. 글마다 신고 버튼을 두는 일은 준비 중입니다.
- **푸터 정리**: 이용약관·운영정책·개인정보처리방침을 한 칸에 모아, 알콜위키 밖의 어느 화면에서나 곧장 닿게 했습니다.
