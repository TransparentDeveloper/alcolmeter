...
classDiagram
    ...
    class Wash {
        ...
        +feed(volume, sugar, maxConcentration) Wash
        +ferment(maxAbv) Wash
    }
...
- 각 단계: 물·당 투입(`Wash.feed`) → 당 농도가 한계를 넘으면 초과분은 **단맛(잔당)으로 손실** → 내성까지 발효(`Wash.ferment`)
...
- **유도(초기 규칙)**: 물·누룩은 1단계(밑술)에 전량, 쌀은 N단계 균등 분할. 더 정교한 유도(현재 상태 기반 가변 분배)는 추후.
- ⚠️ 임시 계수 탓에 물이 너무 적은 구간에선 도수가 비단조일 수 있다(저수분 → 단계 농도↑ → 손실↑). 캘리브레이션 대상.
